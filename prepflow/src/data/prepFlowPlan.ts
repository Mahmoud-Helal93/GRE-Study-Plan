// PrepFlow full 8-week GRE study plan — single source of truth
// Types match the uploaded plan exactly; helpers support the rest of the app.
...
// Backward-compat aliases used throughout the app
export type TaskType = PrepFlowTask["type"];
export type BlockType = PrepFlowBlock["type"];
export type PlanTask = PrepFlowTask;
export type PlanSection = PrepFlowSection;
export type PlanBlock = PrepFlowBlock;
export interface PlanLink { text?: string; label?: string; url: string; }
...
export function getAllTasks(plan: PlanBlock[]): PlanTask[] {
  return plan.flatMap((block) => block.sections.flatMap((s) => s.tasks));
}

export function getBlockTasks(block: PlanBlock): PlanTask[] {
  return block.sections.flatMap((s) => s.tasks);
}

export function getTasksByBlock(plan: PlanBlock[], blockId: string): PlanTask[] {
  const block = plan.find((b) => b.id === blockId);
  if (!block) return [];
  return block.sections.flatMap((s) => s.tasks);
}
...
export function getBlockProgress(
  ...
  const tasks = getTasksByBlock(plan, blockId).filter((t) => !t.optional);
...
export function getOverallProgress(
  ...
  const required = getAllTasks(plan).filter((t) => !t.optional);
  const completed = required.filter((t) => completedIds.has(t.id)).length;
...
export function getWeekProgress(
  ...
  plan: PlanBlock[],
  completedIds: Set<string>
): { completed: number; total: number; pct: number; weekTitle: string } {
  const blocks = plan.filter((b) => b.week === weekNumber);
  const required = blocks
    .flatMap((b) => b.sections.flatMap((s) => s.tasks))
    .filter((t) => !t.optional);
  const completed = required.filter((t) => completedIds.has(t.id)).length;
  const total = required.length;
  return {
    ...
    weekTitle: weekNumber === 0 ? "Before You Begin" : `Week ${weekNumber}`,
  };
}
...
export function getIncompleteCount(
  ...
  return getAllTasks(plan)
    .filter((t) => !t.optional && !completedIds.has(t.id)).length;
}

export function getNextIncompleteBlock(
  plan: PlanBlock[],
  completedIds: Set<string>
): PlanBlock | null {
  return (
    plan.find((block) => {
      const required = block.sections
        .flatMap((s) => s.tasks)
        .filter((t) => !t.optional);
...
export interface WeekIncompleteStat {
  weekNumber: number;
  weekTitle: string;
  incomplete: number;
  total: number;
...
export function getWeekIncompleteStats(
  plan: PlanBlock[],
  completedIds: Set<string>
): WeekIncompleteStat[] {
  const map = new Map<number, { incomplete: number; total: number }>();
  for (const block of plan) {
    if (!map.has(block.week)) map.set(block.week, { incomplete: 0, total: 0 });
    const stat = map.get(block.week)!;
    for (const section of block.sections) {
      for (const task of section.tasks) {
        if (task.optional) continue;
        stat.total++;
        if (!completedIds.has(task.id)) stat.incomplete++;
      }
    }
  }
  return Array.from(map.entries())
    ...
    .map(([week, stat]) => ({
      weekNumber: week,
      weekTitle: week === 0 ? "Before You Begin" : `Week ${week}`,
      ...stat,
    }));
}
...
export function searchPlan(plan: PlanBlock[], query: string): SearchResult[] {
  ...
  for (const block of plan) {
    for (const section of block.sections) {
      for (const task of section.tasks) {
        ...
        const matchesBlock = block.title.toLowerCase().includes(q);
        const matchesCategory = section.category.toLowerCase().includes(q);
        const matchesLink = task.links?.some((l) =>
          (l.text ?? "").toLowerCase().includes(q)
        );
        if (matchesTask || matchesBlock || matchesCategory || matchesLink) {
          results.push({ block, section, task });
        }
      }
    }
  }
  ...
  for (const block of plan) {
    for (const section of block.sections) {
      ...
      for (const task of section.tasks) {
        if (task.optional) continue;
        stats[cat].total++;
        if (completedIds.has(task.id)) stats[cat].completed++;
      }
    }
  }
  return stats;
...
export function groupBlocksByWeek(
  plan: PlanBlock[]
): Array<{ weekLabel: string; blocks: PlanBlock[] }> {
  ...
  return sorted.map(([week, blocks]) => ({
    weekLabel: week === 0 ? "Before You Begin" : `Week ${week}`,
    blocks,
  }));
}
...
export function getPlanGroups(plan: PlanBlock[]): PlanGroup[] {
  const setupItems: PlanBlock[] = [];
  const weekMap = new Map<number, PlanBlock[]>();

  for (const block of plan) {
    if (block.id.startsWith("setup")) {
      setupItems.push(block);
    } else {
      const weekMatch = block.id.match(/^w(\d+)/);
      if (weekMatch) {
        const weekNum = parseInt(weekMatch[1], 10);
        if (!weekMap.has(weekNum)) weekMap.set(weekNum, []);
        weekMap.get(weekNum)!.push(block);
      }
    }
  }
  ...
  if (setupItems.length > 0) {
    groups.push({ id: "setup", title: "Before You Begin", items: setupItems });
  }

  const sortedWeeks = Array.from(weekMap.keys()).sort((a, b) => a - b);
  for (const weekNum of sortedWeeks) {
    const blocks = weekMap.get(weekNum)!;
    const practiceTest = blocks.filter((b) => b.id === "w8-practice-test");
    const regularDays = blocks.filter((b) => b.id !== "w8-practice-test");
    regularDays.sort((a, b) => a.day - b.day);
    groups.push({
      id: `week-${weekNum}`,
      title: `Week ${weekNum}`,
      items: [...regularDays, ...practiceTest],
    });
  }
