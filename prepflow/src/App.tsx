      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden min-w-0">
        ...
        <div className="hidden lg:flex items-center px-6 pt-3.5 pb-3 shrink-0 gap-4">
          ...
          <div className="flex items-stretch gap-2 flex-1 min-w-0">
            ...
            {/* Overall */}
            <div className="flex flex-col justify-center bg-white border border-gray-100 rounded-xl px-3 py-1.5 shadow-[0_1px_3px_0_rgba(0,0,0,0.04)] min-w-0">
              <p className="text-[9px] font-bold uppercase tracking-[0.13em] text-slate-400">Overall</p>
              <div className="flex items-baseline gap-1.5 mt-0.5">
                <span className={`text-base font-bold tabular-nums leading-none ${overall.pct === 100 ? "text-emerald-600" : "text-slate-900"}`}>
                  {overall.pct}%
                </span>
                <span className="text-[10px] text-slate-400 font-medium tabular-nums">{overall.completed}/{overall.total}</span>
              </div>
              <div className="mt-1.5 h-1 w-full rounded-full bg-slate-100 overflow-hidden">
                <div className={`h-full rounded-full transition-all duration-500 ${overall.pct === 100 ? "bg-emerald-500" : "bg-violet-500"}`} style={{ width: `${overall.pct}%` }} />
              </div>
            </div>

            {/* Today */}
            <div className="flex flex-col justify-center bg-white border border-violet-100 rounded-xl px-3 py-1.5 shadow-[0_1px_3px_0_rgba(0,0,0,0.04)] min-w-0 flex-1">
              <p className="text-[9px] font-bold uppercase tracking-[0.13em] text-slate-400">Today</p>
              <div className="flex items-baseline gap-1.5 mt-0.5">
                <span className={`text-base font-bold tabular-nums leading-none ${blockProgress.pct === 100 ? "text-emerald-600" : "text-violet-600"}`}>
                  {blockProgress.pct}%
                </span>
                <span className="text-[10px] text-slate-500 font-semibold truncate">{selectedBlock?.label ?? "—"}</span>
              </div>
              <div className="mt-1.5 h-1 w-full rounded-full bg-slate-100 overflow-hidden">
                <div className={`h-full rounded-full transition-all duration-500 ${blockProgress.pct === 100 ? "bg-emerald-500" : "bg-violet-500"}`} style={{ width: `${blockProgress.pct}%` }} />
              </div>
            </div>

            {/* This Week */}
            <div className="flex flex-col justify-center bg-white border border-violet-100 rounded-xl px-3 py-1.5 shadow-[0_1px_3px_0_rgba(0,0,0,0.04)] min-w-0 flex-1">
              <p className="text-[9px] font-bold uppercase tracking-[0.13em] text-slate-400">This Week</p>
              <div className="flex items-baseline gap-1.5 mt-0.5">
                <span className={`text-base font-bold tabular-nums leading-none ${weekProgress.pct === 100 ? "text-emerald-600" : "text-violet-600"}`}>
                  {weekProgress.pct}%
                </span>
                <span className="text-[10px] text-slate-500 font-semibold truncate">{weekProgress.weekTitle}</span>
              </div>
              <div className="mt-1.5 h-1 w-full rounded-full bg-slate-100 overflow-hidden">
                <div className={`h-full rounded-full transition-all duration-500 ${weekProgress.pct === 100 ? "bg-emerald-500" : "bg-violet-500"}`} style={{ width: `${weekProgress.pct}%` }} />
              </div>
            </div>

            {/* Remaining */}
            <div className="flex flex-col justify-center bg-white border border-gray-100 rounded-xl px-3 py-1.5 shadow-[0_1px_3px_0_rgba(0,0,0,0.04)] min-w-0">
              <p className="text-[9px] font-bold uppercase tracking-[0.13em] text-slate-400">Remaining</p>
              <div className="flex items-baseline gap-1.5 mt-0.5">
                <span className={`text-base font-bold tabular-nums leading-none ${incompleteOverall === 0 ? "text-emerald-600" : "text-slate-900"}`}>
                  {incompleteOverall}
                </span>
                {incompleteWeek > 0
                  ? <span className="text-[10px] text-amber-500 font-semibold tabular-nums">{incompleteWeek} this wk</span>
                  : <span className="text-[10px] text-emerald-500 font-semibold">{incompleteOverall === 0 ? "All done!" : "Wk done!"}</span>
                }
              </div>
              <div className="mt-1.5 h-1 w-full rounded-full bg-slate-100 overflow-hidden">
                <div className={`h-full rounded-full transition-all duration-500 ${incompleteOverall === 0 ? "bg-emerald-500" : "bg-amber-400"}`}
                  style={{ width: `${overall.total > 0 ? Math.round((overall.total - incompleteOverall) / overall.total * 100) : 0}%` }} />
              </div>
            </div>
          </div>
